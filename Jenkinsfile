        ...
        // }
         ...
         stage('Publish Test Reports') {
            steps {
                ...
                junit 'test-results/junit.xml' // Path to the JUnit XML file generated by Jest

                // Publish Cobertura code coverage results
                // Ensure the 'Cobertura Plugin' is installed in Jenkins
                // Ensure Jest outputs to 'coverage/cobertura-coverage.xml'
                // You might need to adjust 'coberturaReportFile' if your Jest output path is different
                recordCoverage publishers: [
                    [$class: 'CoberturaCoveragePublisher',
                     autoUpdateHealth: true,
                     autoUpdateStability: true,
                     coberturaReportFile: 'coverage/cobertura-coverage.xml',
                     onlyConsideringStoredBuilds: false,
                     failUnhealthy: false, // Don't fail the build just because coverage is low
                     failUnstable: false, // Don't make the build unstable just because coverage is low
                     lineCoverageTargets: '0, 80, 90', // Targets for lines (unhealthy, unstable, healthy)
